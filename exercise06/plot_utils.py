...
def plot_trajectories_pendulum_on_cart(s_traj, a_traj, amax, amin):
    N = a_traj.size
    ts = np.arange(0, N+1)
    ...
    na = 1
#     latexify()
    state_labels = [r'$p_k$', r'$\theta_k$', r'$v_k$', r'$\omega_k$']

    plt.figure(figsize=(5, 10))

    for i in range(ns):
        plt.subplot(ns+na, 1, i+1)

        plt.plot(ts, s_traj[i, :].T, '-', alpha=0.7)
        plt.ylabel(state_labels[i])
        plt.xticks(np.arange(0, N+1, 5))
        plt.grid()

    plt.subplot(ns+na, 1, ns+na)

    plt.step(ts[:-1], a_traj.T, alpha=0.7, where='post')
    plt.axhline(amax, color='gray', linestyle='dashed')
    plt.axhline(amin, color='gray', linestyle='dashed')
    plt.grid()
...
def plot_trajectories(s_trajs, a_trajs, labels, states_labels=None, action_labels=None):
